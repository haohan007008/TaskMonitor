...
<div class="pd-20">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<c:forEach items="${templatelist}" var="p">
				<tr class="text-c">
					<td><input name="" type="checkbox" value=""></td>
					<td>${p.tempId}</td>
					<td>${p.sysName}</td>
					<td class="text-l">${p.templateName}</td>
					<td class="text-c">${p.srcConn}</td>
					<td>${p.queueName}</td>
					<td>${p.statusName}</td>
					<td>${p.maxzlID}</td>
					<td>${p.description}</td>
...
function picture_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
		    type:'post',
		    url:'${pageContext.request.contextPath}/datapush/delds.html?dsId='+id,
		    dataType: 'json',
